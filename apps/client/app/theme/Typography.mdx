...
export function Template({ prefix }) {
  const target = Object.entries(typography)
    .filter(([key]) => key.startsWith(prefix))
    .map(([_, value]) => value);
  const first = target[0];
  return (
    <Typeset
      fontSizes={target.map((v) => v.fontSize)}
      fontWeights={first.fontWeight}
      lineHeights={first.lineHeight}
